...
ENTITY_RESPONSE=$(curl -sS -X 'GET' \
  "https://tna.preservica.com/api/entity/content-objects/$ASSET_ID" \
  -H 'accept: application/xml;charset=UTF-8' \
  -H "Preservica-Access-Token: $TOKEN")


ENTITY_REF=$(echo $ENTITY_RESPONSE | xmllint --xpath '/*[local-name()="EntityResponse"]/*[local-name()="ContentObject"]/*[local-name()="Parent"]/text()' -)
