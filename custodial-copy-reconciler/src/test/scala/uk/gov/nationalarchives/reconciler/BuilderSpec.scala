...
class BuilderSpec extends AnyFlatSpec:
  ...
  "Builder run" should ", given a ContentObject, return all Preservica CoRows regardless of generation type and version" in {
    ...
    val preservicaCoRows = Builder[IO](preservicaClient).run(entityIds).compile.toList.unsafeRunSync()

    preservicaCoRows.length should equal(3)

    preservicaCoRows.contains(CoRow(co1Ref, potentialParentRef, "co1FileFixity", effectiveDate.toOffsetDateTime)) should equal(true)
    preservicaCoRows.contains(CoRow(co1Ref, potentialParentRef, "co1FileFixity2", effectiveDate.toOffsetDateTime)) should equal(true)
    preservicaCoRows.contains(CoRow(co2Ref, potentialParentRef, "co2FileFixity", effectiveDate.toOffsetDateTime)) should equal(true)
  }
