...
class DatabaseSpec extends AnyFlatSpec with BeforeAndAfterEach:
  ...
  "findAllMissingCOs" should s"should return a message for each CO if the checksums don't match" in {
    ...
    createPreservicaCOsTable()
    createOcflCOsTable()
    (createPSCoRow(coRef, ioRef, preservicaChecksum, createdDate) >> createCoRow(coRefTwo, ioRef, ocflChecksum, createdDate)).unsafeRunSync()

    val result = Database[IO](mutex).findAllMissingCOs(createdDate.plusDays(1)).unsafeRunSync()
