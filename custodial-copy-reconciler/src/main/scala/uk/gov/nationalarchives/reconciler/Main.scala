...
object Main extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] =
    for {
      ...
      _ <- IO(runReconciler(client, ocflService, semaphore, eventBridgeClient).handleErrorWith(err => Stream.eval(logError(err))))
    } yield ExitCode.Success

  def runReconciler(client: EntityClient[IO, Fs2Streams[IO]], ocflService: OcflService, semaphore: Semaphore[IO], eventBridgeClient: DAEventBridgeClient[IO])(
