...
object Main extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] =
    for {
      ...
      _ <- runReconciler(client, OcflService(config, startTime), eventBridgeClient, Database[IO](mutex), startTime).handleErrorWith(logError)
    } yield ExitCode.Success

  def runReconciler(client: EntityClient[IO, Fs2Streams[IO]], ocflService: OcflService[IO], eventBridgeClient: DAEventBridgeClient[IO], database: Database[IO], startTime: OffsetDateTime)(using
    ...
    val endDate = startTime.minusDays(configuration.config.daysToIgnore)
    val builder = Builder[IO](client, endDate)
    ...
    def getEntities: Stream[IO, CoRow] =
      client.getAllAssetIds(configuration.config.maxConcurrency)
