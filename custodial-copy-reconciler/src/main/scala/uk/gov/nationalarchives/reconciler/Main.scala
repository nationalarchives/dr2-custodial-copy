...
object Main extends IOApp {
  ...
  def runReconciler(client: EntityClient[IO, Fs2Streams[IO]], ocflService: OcflService[IO], eventBridgeClient: DAEventBridgeClient[IO])(using
    ...
    val ocfl = ocflService.getAllObjectFiles.chunkN(10000)
    ...
    def updatedSince(start: Int): IO[Seq[Entities.Entity]] = client.entitiesUpdatedSince(startOfEpoch, start)

    def getEntities(start: Int = 0): Stream[IO, CoRow] = 
      Stream.unfoldEval(0) { start =>
        updatedSince(start).flatMap {
          case Nil => IO.none
          case entities => 
            entities
              .filter(_.entityType.contains(ContentObject))
              .flatTraverse(builder.run)
    ...
    IO.bothOutcome(ocfl, ps).map {
      case (Succeeded(_), Succeeded(_)) => IO.println("All fine") >> IO.unit
