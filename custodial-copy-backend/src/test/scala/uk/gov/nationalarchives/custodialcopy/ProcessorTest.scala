...
class ProcessorTest extends AnyFlatSpec with MockitoSugar {
  ...
  "process" should "throw an Exception if XML string passed to validator is invalid" in {
    ...
    val res = utils.processMessage.attempt.unsafeRunSync()

    res.left.foreach(
      _.getMessage should equal(
        """cvc-complex-type.2.4.a: Invalid content was found starting with element '{"http://preservica.com/XIP/v7.7":InvalidTag}'. """ +
          """One of '{"http://preservica.com/XIP/v7.7":Parent}' is expected."""
      )
    )
  ...
  "process" should "return the ref of the message passed to it" in {
    ...
    val idResponse = utils.processMessage.unsafeRunSync()
